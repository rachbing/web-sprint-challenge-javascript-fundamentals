...
// That variable was not inside that function's scope, so the function looked outward for context and found const internal in the outer scope.
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(zooAnimals){
    const lowNames = zooAnimals.map(item => item.animal_name.toLowerCase())
    return lowNames
  }
  ...
  */
// step one: put array zooAnimals as argument in function.
// step 2: create new array
// step 3: use .filter() to filter animals with pop < 5 into new array
// return the new array
  ...
  */
  ...
  // zooAnimals as argument
  // declare new var = .reduce
  // fill in acc and item, set initial value to 0
  // return final value / totalPop
...
// enter 3 parameters as arguments
...
class CuboidMakerTwo{
  constructor (twoAtr){
    this.length = twoAtr.length;
    this.width = twoAtr.width;
    this.height = twoAtr.height;
  } volume() {
    return this.length * this.width * this.height
  } surfaceArea() {
    return 2 * (this.length * this.width + this.length * this.height + this.width * this.height)
